...
//    final RepositoryInformation metadata;  // 不兼容 solon
...
//        this.metadata = metadata; // 不兼容 solon
        this.repositoryInterface = repositoryInterface;
        this.domainType = domainType;
        this.superType = superType;
//        this.interfaceInternalName = Type.getInternalName(metadata.getRepositoryInterface()); // 不兼容 solon
...
//        this.entityInternalName = Type.getInternalName(metadata.getDomainType()); // 不兼容 solon
        this.entityInternalName = Type.getInternalName(domainType);
        this.sqlClientDescriptor = Type.getDescriptor(sqlClientType);
//        Class<?> repositoryInterface = metadata.getRepositoryInterface(); // 不兼容 solon
...
//        mv.visitLdcInsn(Type.getType(metadata.getDomainType())); // 不兼容 solon
        mv.visitMethodInsn(
...
//        mv.visitLdcInsn(Type.getType(metadata.getDomainType())); // 不兼容 solon
        ...
        for (MethodCodeWriter writer : autoGenMethodCodeWriters) {
            mv.visitVarInsn(Opcodes.ALOAD, 0);
            mv.visitVarInsn(Opcodes.ALOAD, 1);
//            mv.visitLdcInsn(Type.getType(metadata.getRepositoryInterface()));  // 不兼容 solon
